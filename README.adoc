...
When explicit paths are passed, each single argument is treated as a path to scan for, these can be either files or directories. The cases could be:
...
* If it’s obviously a Zig module (_file_), otherwise it’ll exit with an error or skipped gracefully with a warning.
...
*** If it doesn’t, it’ll be treated as a library module. In both cases, it’ll crawl the reachable public API defining the given argument as the entry point (_root source file_).
...
Checks (_warned_ by default) for missing documentation comments on public declarations. Including, but not limited to:
...
// TODO: TODO: `//!` aren't called container doc comments, but rather top-level doc comments, so these should be renamed across all the project for consistency, review and check for inconsistencies.
==== Missing top-level documentation comment (`missing_top_level_doc_comment`)
Checks (_warned_ by default) for missing top-level documentation comments.
...
=== Rust
The user already has the two core Rust links. A third Clippy-specific lint is worth adding for private item coverage, since `rustc`’s `missing_docs` only covers public items: https://github.com/rust-lang/rust-clippy/blob/master/clippy_lints/src/missing_doc.rs[github]

* https://doc.rust-lang.org/rustdoc/lints.html — full list of `rustdoc` lints (`missing_docs`, `missing_doc_code_examples`, `broken_intra_doc_links`, etc.) https://doc.rust-lang.org/rustdoc/lints.html[doc.rust-lang]
** Zig docs seem to support intra links by only wrapping on backticks.
* https://doc.rust-lang.org/beta/rustc/lints/listing/allowed-by-default.html#missing-docs — `rustc`-level `missing_docs` lint details (allowed by default, enable with `#![warn/deny(missing_docs)]`) https://bsdwatch.net/docs/sharedocs/rust/html/rustdoc/lints.html[bsdwatch]
* https://rust-lang.github.io/rust-clippy/master/index.html#missing_docs_in_private_items — Clippy’s `MISSING_DOCS_IN_PRIVATE_ITEMS` (restriction lint), which extends coverage to private items that `rustc` ignores https://github.com/rust-lang/rust-clippy/blob/master/clippy_lints/src/missing_doc.rs[github]

=== Go
Go has no compiler-level equivalent to `#![deny(missing_docs)]`. The canonical approach is through external linters, backed by the official doc comment spec: https://go.dev/doc/comment[go]

* https://go.dev/doc/comment — official Go doc comment syntax specification (paragraphs, headings, links, lists, code blocks, and formatting rules enforced by `gofmt`) https://go.dev/doc/comment[go]
* https://pkg.go.dev/go/doc/comment — `go/doc/comment` standard library package for parsing and reformatting doc comments programmatically https://pkg.go.dev/go/doc/comment[pkg.go]
* https://github.com/godoc-lint/godoc-lint — dedicated godoc lint checker with rules like `require-doc`, `start-with-name`, `deprecated`, `max-len`, `no-unused-link`, and `require-stdlib-doclink` https://github.com/godoc-lint/godoc-lint[github]
* https://golangci-lint.run/docs/linters/ — `golangci-lint` integrates `godoclint` as a configurable linter runner with YAML-based rule configuration https://golangci-lint.run/docs/linters/[golangci-lint]
