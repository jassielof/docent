...
fn proseTitleForRule(comptime rule_name: []const u8) []const u8 {
    if (std.mem.eql(
        ...
        rule_name,
        "missing_doc_comment",
    )) return rules.doc.missing_doc_comment.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "missing_doctest",
    )) return rules.doc.missing_doctest.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "private_doctest",
    )) return rules.doc.private_doctest.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "blank_doc_comment",
    )) return rules.doc.blank_doc_comment.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "missing_summary_terminal_punctuation",
    )) return rules.doc.missing_summary_terminal_punctuation.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "trailing_blank_doc_comment",
    )) return rules.doc.trailing_blank_doc_comment.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "doctest_naming_mismatch",
    )) return rules.doc.doctest_naming_mismatch.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "invalid_leading_phrase",
    )) return rules.doc.invalid_leading_phrase.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
    ...
    )) return rules.complexity.cognitive.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "cyclomatic_complexity",
    )) return rules.complexity.cyclomatic.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "max_fun_params",
    )) return rules.size.max_fun_params.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "identifier_case",
    )) return rules.style.identifier_case.prose_title;
    if (std.mem.eql(
        u8,
        rule_name,
        "line_length_limit",
    )) return rules.size.line_length_limit.prose_title;
