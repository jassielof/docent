...
pub fn Rule(comptime default_severity: severity.Level, comptime OptionsType: type, comptime default_scan: scanning.Modes) type {
    return struct {
...
pub fn applyRunScanMode(self: anytype, mode: scanning.Modes) void {
    self.scan_mode = mode;
    inline for (std.meta.fields(@TypeOf(self.*))) |field| {
        if (comptime std.mem.eql(u8, field.name, "scan_mode")) continue;
