...
test "detects blank /// with spaces" {
    var result = try harness.lintRuleFixtureDisplay(
        ns,
        &.{"blank_doc_empty_with_spaces.zig"},
        warn,
        .{},
        null,
    );
...
test "detects fully blank multiline /// comment block once" {
    var result = try harness.lintRuleFixtureDisplay(
        ns,
        &.{"fully_blank_multiline_doc_comment.zig"},
        warn,
        .{},
        null,
    );
...
test "no diagnostic for multiline block with at least one non-empty line" {
    var result = try harness.lintRuleFixtureDisplay(
        ns,
        &.{"partially_empty_doc_comment.zig"},
        warn,
        .{},
        null,
    );
...
test "member re-export does not trigger whole-module blank check" {
    var result = try harness.lintRuleFixtureDisplay(
        ns,
        &.{"blank_doc_member_reexport_skip.zig"},
        warn,
        .{},
        null,
    );
