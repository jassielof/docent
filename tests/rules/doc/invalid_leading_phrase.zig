...
fn lint(
    ...
    return harness.lintRuleFixtureConfigured(
        ns,
        &.{fixture},
        rule_set,
        lint_options,
...
fn expectLeadingPhraseSubject(
    result: docent.LintResult,
    kind: docent.Diagnostic.SubjectKind,
) !void {
...
test "accepts identifier-first summary on a function" {
    var result = try lint(
        "leading_phrase_identifier_first.zig",
        warn,
        public_surface,
        null,
    );
...
test "accepts kind word before backticked identifier" {
    var result = try lint(
        "leading_phrase_kind_before_backtick.zig",
        warn,
        public_surface,
        null,
    );
...
test "warns on identifier case mismatch for declarations" {
    var result = try lint(
        "leading_phrase_case_mismatch.zig",
        warn,
        public_surface,
        null,
    );
...
test "accepts struct documented with article and trailing kind" {
    var result = try lint(
        "leading_phrase_struct_article_kind.zig",
        warn,
        public_surface,
        null,
    );
...
test "accepts error set with article and trailing kind" {
    var result = try lint(
        "leading_phrase_error_set_article_kind.zig",
        warn,
        public_surface,
        null,
    );
...
test "accepts constant identifier first" {
    var result = try lint(
        "leading_phrase_constant_identifier_first.zig",
        warn,
        public_surface,
        null,
    );
...
test "module doc accepts case-insensitive name with kind word" {
    var result = try lint(
        "leading_phrase_module_kind_accept.zig",
        warn,
        .{ .scan_mode = .public_api_surface, .module_name = "docent" },
        null,
    );
    defer result.deinit();
    try utils.expectRuleAbsent(result, "invalid_leading_phrase");
...
test "module doc warns when name is absent" {
    var result = try lint(
        "leading_phrase_module_missing_name.zig",
        warn,
        .{ .scan_mode = .public_api_surface, .module_name = "docent" },
        null,
    );
...
test "no diagnostic for undocumented or unresolved blocks" {
    var result = try lint(
        "leading_phrase_unattached_doc.zig",
        warn,
        public_surface,
        null,
    );
...
test "accepts enumerator identifier first" {
    var result = try lint(
        "leading_phrase_enumerator_ok.zig",
        warn,
        public_surface,
        null,
    );
...
test "no diagnostic for private function doc comment" {
    var result = try lint(
        "leading_phrase_private_fn_skipped.zig",
        warn,
        public_surface,
        null,
    );
...
test "warns on public function when public_api_only" {
    var result = try lint(
        "leading_phrase_public_fn_warn.zig",
        warn,
        public_surface,
        null,
    );
...
test "private function checked when public_api_only is false" {
    var result = try lint(
        "leading_phrase_private_fn_all_mode.zig",
        warn,
        reachability,
        null,
    );
...
test "require_kind accepts kind-before identifier summary" {
    var result = try lint(
        "leading_phrase_strict_accept.zig",
        warn,
        public_surface,
        setRequireKind,
    );
...
test "require_article accepts summary with article" {
    var result = try lint(
        "leading_phrase_require_article_accept.zig",
        warn,
        public_surface,
        setRequireArticle,
    );
...
test "require_backticks rejects bare identifier" {
    var result = try lint(
        "leading_phrase_require_backticks_reject.zig",
        warn,
        public_surface,
        setRequireBackticks,
    );
...
test "require_backticks accepts backticked identifier" {
    var result = try lint(
        "leading_phrase_require_backticks_accept.zig",
        warn,
        public_surface,
        setRequireBackticks,
    );
