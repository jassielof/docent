...
test "godoc preset loads documentation grammar and zig naming defaults" {
    ...
    try std.testing.expect(cfg.docs.scan_mode == .public_api_surface);
    try std.testing.expect(cfg.docs.invalid_leading_phrase.options.mode == .canonical);
    try std.testing.expect(!cfg.docs.invalid_leading_phrase.options.require_article);
    try std.testing.expect(cfg.docs.missing_doctest.level == .allow);
