...
fn presetConfigPath(
    ...
    const rel = try std.fmt.allocPrint(
        allocator,
        "tests/fixtures/config/presets/{s}.toml",
        .{name},
    );
...
test "tiger style preset enforces snake_case and line length forbid" {
    ...
    const config_path = try presetConfigPath(
        allocator,
        io,
        "tiger-style",
    );
    ...
    var cfg = try docent.config.loadConfig(
        allocator,
        io,
        config_path,
    );
    ...
    try std.testing.expectEqual(casing.Style.snake, style.identifier_case.options.struct_file_case);
